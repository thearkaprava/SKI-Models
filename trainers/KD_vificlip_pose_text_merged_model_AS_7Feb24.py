...
class ClipTextEncoder(nn.Module):
    def __init__(self, clip_model2):
        super().__init__()
        self.transformer = clip_model2.transformer
        self.positional_embedding = clip_model2.positional_embedding
        self.ln_final = clip_model2.ln_final
        self.text_projection = clip_model2.text_projection
        self.dtype = clip_model2.dtype
...
class ViFiCLIP(nn.Module):
    ...
    def forward(self, image, hyperformer_data=None): #, pose_embedding):
        ...
        if hyperformer_data is not None:
            ...
            clip_text_features = self.clip_text_encoder(prompts, tokenized_prompts)
...
def returnCLIP(config, logger=None,
    ...
    for name, param in model.named_parameters():
        # ## Knowledge Distillation Image Text to Pose Text
        # if ('text_encoder' in name and 'clip' not in name) or ('image_encoder' in name):
        #     param.requires_grad_(False)

        ## Knowledge Distillation Pose Text to Image Text
        if ('clip_text_encoder' in name) or ('hyperformer_model' in name):
            param.requires_grad_(False)
