...
class SKCLIP(nn.Module):
    ...
    def forward(self, image, hyperformer_data=None): #, pose_embedding):
        ...
        if hyperformer_data is not None:
            # pose_embedding, pose_features = self.hyperformer_model(hyperformer_data)
            pose_embedding = self.hyperformer_model(hyperformer_data)
            # return image_features, text_features, pose_embedding
            # pose_text_logits = pose_embedding @ text_features.t()
            # return pose_text_logits
            return pose_embedding, text_features #, pose_features
        else:
...
def returnCLIP(config, logger=None,
    ...
    model = SKCLIP(config, class_names, clip_model, logger, args)
