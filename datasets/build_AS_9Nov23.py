...
class BaseDataset(Dataset, metaclass=ABCMeta):
    def __init__(self,
        ...
        #     #     self.sample_name = ['train_' + str(i) for i in range(len(self.data))]
        #     #     self.filename = [x.decode('utf-8') for x in npz_data['files_train']]
        #     # elif self.split == 'test':
        #     #     self.data = npz_data['x_test']
        #     #     self.label = np.where(npz_data['y_test'] > 0)[1]
        #     #     self.sample_name = ['test_' + str(i) for i in range(len(self.data))]
        #     #     self.filename = [x.decode('utf-8') for x in npz_data['files_test']]
        #     # else:
    ...
    def load_annotations(self):
        ...
        with open(self.ann_file, 'r') as fin:
            for line in fin:
                ...
                # hformer_embeds = self.hyperformer_embeds[video_identifier]
                # counter = 0
                # if len(video_identifier):
                    # hformer_data = self.hyperformer_data[video_identifier]
                    # print("Video Found")
                    # video_infos.append(dict(filename=filename, label=label, hformer_data=hformer_data, tar=self.use_tar_format))
                # else:  
                #     counter += 1
                #     print(f"{counter} VIDEO NAME = {video_identifier}")
