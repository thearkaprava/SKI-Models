...
class BaseDataset(Dataset, metaclass=ABCMeta):
    def __init__(self,
        ...
        if 'ntu' in self.ann_file:
            ...
            # data_path = '/data/ntu/foundation_skeleton_data_merged.npz'
            npz_data = np.load(data_path)
            hyperformer_data_train = npz_data['x_train']
            hyperformer_label_train = np.where(npz_data['y_train'] > 0)[1]
            # self.sample_name = ['train_' + str(i) for i in range(len(self.data))]

            train_filenames = [x.decode('utf-8') if isinstance(x, bytes) else x for x in npz_data['files_train']]
            test_filenames = [x.decode('utf-8') if isinstance(x, bytes) else x for x in npz_data['files_test']]
            ...
            hyperformer_filename = np.concatenate((train_filenames, test_filenames), axis=0)
        ...
        with open(self.ann_file, 'r') as fin:
            for line in fin:
                ...
                if self.async_loading:
                    ...
                    hyperformer_data = self.hyperformer_data[random_filename_same_action]

                else:
                    hyperformer_data = self.hyperformer_data[video_identifier]
                # breakpoint()
                video_infos.append(dict(filename=filename, label=label, hyperformer_data=hyperformer_data, tar=self.use_tar_format))
        return video_infos
...
def build_dataloader(logger, config):
    ...
    train_pipeline = [
        dict(type='Collect', keys=['hyperformer_data', 'label'], meta_keys=[]),
        dict(type='ToTensor', keys=['hyperformer_data', 'label']),        
    ]
    train_data = VideoDataset(ann_file=config.DATA.TRAIN_FILE, data_prefix=config.DATA.ROOT,
                              labels_file=config.DATA.LABEL_LIST, pipeline=train_pipeline, async_loading=config.DATA.ASYNC_LOADING)
...
def build_dataloader_no_ddp_bone(logger, config):
    print("Bone")
    scale_resize = int(256 / 224 * config.DATA.INPUT_SIZE)
